...
const PasswordChangeForm: React.FC<PasswordChangeFormProps> = ({ onClose }) => {
    // Removed oldPassword state as it's not used by supabase.auth.updateUser directly
    const [newPassword, setNewPassword] = useState('');
    const [confirmNewPassword, setConfirmNewPassword] = useState('');
    const [loading, setLoading] = useState(false);
    // Removed showOldPassword state
    ...
    const handlePasswordChange = async (e: React.FormEvent) => {
        ...
        try {
            // supabase.auth.updateUser changes password based on current session's authentication.
            // It does NOT verify old password directly. If the session is old, it might fail.
            const { error: updateError } = await supabase.auth.updateUser({ password: newPassword });

            if (updateError) {
                // Improved error handling based on common Supabase messages
                if (updateError.message.includes("Invalid login credentials") || updateError.message.includes("AuthApiError: Invalid Refresh Token") || updateError.message.includes("AuthApiError: Session not found")) {
                    toast({ 
                        title: "เซสชันหมดอายุ", 
                        description: "โปรดออกจากระบบและเข้าสู่ระบบใหม่เพื่อยืนยันตัวตน แล้วลองเปลี่ยนรหัสผ่านอีกครั้ง", 
                        variant: "destructive" 
                    });
                    // Consider signOut() here if you want to force re-login automatically
...
const Profile = () => {
  const { user, signOut, refreshUser } = useAuth(); // Destructure refreshUser from useAuth
  ...
  // Memoized profile data display - Moved hooks to top level, before conditional return
  // Ensure date parsing is robust using isNaN(date.getTime())
  ...
  const lastSignInAt = useMemo(() => {
    ...
    // Use toLocaleString for combined date and time style
  ...
  const emailConfirmedColor = useMemo(() => user?.email_confirmed_at ? 'text-green-600' : 'text-orange-600', [user]);
    ...
    try {
      ...
      if (data) {
        ...
        setFullName(data.full_name || '');
          ...
          setProfile({ id: user.id, full_name: user.user_metadata?.full_name || '', created_at: user.created_at, updated_at: user.updated_at });
  ...
  const updateProfile = async () => {
    ...
    try {
        ...
        // Also update user's auth metadata if full_name changed
        ...
        await loadProfile(); 
        await refreshUser(); // Call refreshUser after successful profile update to sync sidebar
    } catch (error: any) {
  ...
  // Render skeleton while loading profile data - Conditional return must be AFTER all hooks
  if (loading) {
  ...
  return (
    <div className="space-y-6 p-4 md:p-8">
      {/* Header Section */}
      <div>
        <h1 className="text-3xl font-bold tracking-tight">โปรไฟล์และการตั้งค่า</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <Card className="lg:col-span-2">
            <CardHeader>
                <div className="flex flex-col sm:flex-row items-center gap-6">
                    ...
                    <div className="flex-1 text-center sm:text-left">
                        <CardTitle className="text-2xl font-bold text-foreground mb-1">{fullName || user?.email}</CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              <Button onClick={updateProfile} disabled={formLoading} className="h-11 text-lg font-bold">
        ...
        <div className="space-y-6">
            ...
            <Card>
                ...
                <CardContent className="space-y-4 text-sm"> 
                    <div className="flex items-center justify-between gap-4"><span className="text-muted-foreground flex items-center gap-2"><CalendarDays size={16}/>เป็นสมาชิกตั้งแต่</span> <span className="font-medium text-foreground">{profileCreatedAt}</span></div>
                    <div className="flex items-center justify-between gap-4"><span className="text-muted-foreground flex items-center gap-2"><History size={16}/>เข้าใช้ล่าสุด</span> <span className="font-medium text-foreground">{lastSignInAt}</span></div>
                    <div className="flex items-center justify-between gap-4"><span className={cn("flex items-center gap-2", emailConfirmedColor)}>{emailConfirmedIcon}สถานะอีเมล</span> <span className={cn("font-medium", emailConfirmedColor)}>{emailConfirmedStatus}</span></div>
                </CardContent>
            </Card>
        </div>
      </div>
      
      {/* Password Change Modal (Dialog for Desktop, Drawer for Mobile) */}
